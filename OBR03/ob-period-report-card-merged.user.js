...
(function () {
  ...
  function createPanel(){
    ...
    panel.innerHTML = `
      ...
      <div id="prcV2HybridPanelBody" style="padding:10px 12px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;">
          ...
          <button class="prcBtn blue" id="pullFlUtilOnly" style="grid-column:span 2;">Pull FL Utilization</button>
          <button class="prcBtn blue" id="pullBeltOnly" style="grid-column:span 2;">Pull Battle of the Belt</button>
    ...
    document.getElementById('pullFlUtilOnly')?.addEventListener('click', () => pullMonitorViaFrames(['flUtil']));
    document.getElementById('pullBeltOnly')?.addEventListener('click', () => pullMonitorViaFrames(['belt']));
    ...
    document.getElementById('pullFclmPeriods')?.addEventListener('click', () => {
      ...
      pullMonitorViaFrames(['flUtil','belt']);
    });
    ...
    if(isDashboard){
      window.addEventListener('message',(event)=>{
        ...
        if(event.data?.type==='PRC_V2_HYBRID_FCLM_PULL'){
          ...
          if(event.data.includeMonitor) pullMonitorViaFrames(['flUtil','belt']);
        }
        if(event.data?.type==='PRC_V2_HYBRID_SOURCE_PULL'){
          ...
          if(source==='flUtil'){
            pullMonitorViaFrames(['flUtil']);
            return;
          }
          if(source==='belt'){
            pullMonitorViaFrames(['belt']);
            return;
          }
          pullFclmInBackground(periods,{...options,includeMonitor:false});
          pullMonitorViaFrames(['flUtil','belt']);
