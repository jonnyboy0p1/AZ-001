...
(function () {
  ...
  const isDashboard =
    ...
    HREF.startsWith('file:///C:/Users/jonavroa/Desktop/OB02/pace.html') ||
  ...
  // OB is the whole; FL / MP / RWC are its parts. Given a flUtil object holding the
  // three individual divert totals, add the combined OB total (= MP + FL + RWC) and
  // each part's share of OB (%), so the Pace tracker can show the whole and the
  // breakdown. Missing parts are ignored in the sum and reported as null.
  function enrichFlUtil(v){
    const obj=v||{};
    const mp=Number.isFinite(obj.mp)?obj.mp:null;
    const fl=Number.isFinite(obj.fl)?obj.fl:null;
    const rwc=Number.isFinite(obj.rwc)?obj.rwc:null;
    const parts=[mp,fl,rwc].filter(n=>n!=null);
    const ob=parts.length?parts.reduce((a,b)=>a+b,0):null;
    const pct=(n)=>(n!=null&&ob)?Math.round((n/ob)*1000)/10:null;
    return {...obj, mp, fl, rwc, ob, mpPct:pct(mp), flPct:pct(fl), rwcPct:pct(rwc)};
  }
  ...
  async function pullMonitorInBackground(periods=['full','p1','p2','p3'],types=['flUtil','belt'],source=null){
    ...
    for(const [key,type,payload] of results){
      ...
      if(type==='flUtil') target.flUtil=enrichFlUtil({...(target.flUtil||{}),...payload.flUtil});
  ...
  function liveMonitorJobs(types,periods){
    const monitorTypes=(types||['flUtil']).filter(t=>['flUtil','belt'].includes(t));
    const wins=(periods&&periods.length)?periods:monitorAllPeriods();
    const jobs=[];
    monitorTypes.forEach(type=>wins.forEach(period=>{
      const url=type==='belt'?buildMonitorBeltUrl(period):buildMonitorFlUtilUrl(period);
      jobs.push({type,period,url,name:monitorTabName(type,period)});
    }));
    return jobs;
  }

  // Open (or refresh) the live monitor graph tabs. Returns true if the browser let
  // at least one tab through; false usually means the pop-up blocker stopped us
  // (use the panel button once, from a click, to grant this page pop-up access).
  // options.periods limits which windows to open (default: every active period);
  // the automatic feed passes ['full'] so just one cumulative-shift tab stays open.
  function openLiveMonitorTabs(types=['flUtil'],options={}){
    if(!isDashboard){
      updatePanel(readBridge(),'Open the dashboard to start the live monitor tabs');
      return false;
    }
    const jobs=liveMonitorJobs(types,options.periods);
    if(!jobs.length) return false;

    let opened=0;
    jobs.forEach(job=>{
      // Reusing a fixed name navigates the existing tab (a refresh) instead of
      // spawning a duplicate; a brand-new tab is what a pop-up blocker may stop.
      const win=window.open(job.url,job.name);
      if(win) opened++;
    });

    try{ window.focus(); }catch(e){} // keep the dashboard in front; tabs stay in back
    GM_setValue('PRC_LIVE_TABS_LAST_OPEN',now());
  ...
  function createPanel(){
    ...
    panel.innerHTML = `
      ...
      <div id="prcV2HybridPanelBody" style="padding:10px 12px;">
        ...
        ${isDashboard?`<button class="prcBtn live" id="openLiveTabs" type="button" style="width:100%;padding:8px;font-size:12px;margin-bottom:8px;">&#9654; Live FL Tabs (15 min &rarr; Pace)</button>`:''}
...
const row=(label,value)=>
    ...
    body.innerHTML=`
      ...
      ${sep()}
      ${sectionLabel('Monitor — FL Utilization (OB)')}
      ${(()=>{ const f=payload.monitorFull?.flUtil||{}; const pc=(n)=>n!=null?` <span style="color:#64748b;">(${n}%)</span>`:''; return `
        ${row('OB = FL+MP+RWC',`<b style="color:#38bdf8;">${fmt(f.ob)}</b> &nbsp; ${fmtAge(f.updatedAt)}`)}
        ${row('FL',`${fmt(f.fl)}${pc(f.flPct)}`)}
        ${row('MP',`${fmt(f.mp)}${pc(f.mpPct)}`)}
        ${row('RWC',`${fmt(f.rwc)}${pc(f.rwcPct)}`)}
      `;})()}
      ${row('Belt P1/P2/P3',`${fmtAge(m.p1?.belt?.updatedAt)} / ${fmtAge(m.p2?.belt?.updatedAt)} / ${fmtAge(m.p3?.belt?.updatedAt)}`)}
    `;
  }
