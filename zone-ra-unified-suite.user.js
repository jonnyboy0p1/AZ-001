...
// @connect      midway-auth.corp.amazon.com
...
(function() {
    ...
    // ==========================================================
    // FCMenu base is user-configurable (region-specific). Change it from the
    // Labor Tracker "My Codes" panel (gear button) if IAD is not your region.
    const FCMENU_BASE = (GM_getValue('lt_fcmenu_base', 'https://fcmenu-iad-regionalized.corp.amazon.com') || '').replace(/\/+$/,'');
    ...
    function ltShowAreaList() {
        ...
        panel.innerHTML='<div class="lt-panel-header"><h3>⏱ My Codes</h3><button class="lt-panel-close" id="lt-panel-x">✕</button></div><div id="lt-active-bar" class="'+(ltActiveCalmCode?'show':'')+'"><span>Active:</span><span class="lt-active-code">'+(ltActiveCalmCode||'')+'</span></div><div class="lt-panel-body" id="lt-panel-content">'+codesHtml+'</div><div style="padding:8px 12px;border-top:1px solid #333;display:flex;gap:6px;"><input id="lt-new-code-input" placeholder="CALM code" style="flex:1;padding:6px 8px;border-radius:6px;border:1px solid #444;background:#1a1a2e;color:#fff;font-size:12px;text-transform:uppercase;font-weight:600;"><button id="lt-add-code-btn" style="padding:6px 12px;border-radius:6px;border:none;background:#4CAF50;color:#fff;font-weight:700;font-size:13px;cursor:pointer;">+</button></div><div style="padding:0 12px 10px;display:flex;gap:6px;"><button id="lt-test-fcmenu" title="Check whether FCMenu is reachable and you are signed in" style="flex:1;padding:7px 10px;border-radius:6px;border:1px solid #0073bb;background:rgba(0,115,187,0.15);color:#5dd8ff;font-weight:600;font-size:12px;cursor:pointer;">🔌 Test FCMenu</button><button id="lt-config-fcmenu" title="Change the FCMenu URL (region)" style="padding:7px 10px;border-radius:6px;border:1px solid #444;background:#1a1a2e;color:#ccc;font-size:13px;cursor:pointer;">⚙️</button></div>';
        ...
        document.getElementById('lt-test-fcmenu').addEventListener('click', ltTestConnection);
        document.getElementById('lt-config-fcmenu').addEventListener('click', ltConfigureFcMenu);
    ...
    // ----------------------------------------------------------
    //  FCMenu connectivity helpers
    //  These endpoints sit behind Midway SSO. When the session is
    //  missing/expired FCMenu 302-redirects to a Midway sign-in page.
    //  GM_xmlhttpRequest can only FOLLOW that redirect if the target
    //  host is in @connect (now added). We then detect the sign-in
    //  page and guide the user to authenticate.
    // ----------------------------------------------------------
    function ltShortUrl(u){ try { return new URL(u).host; } catch(e){ return String(u||''); } }

    // Same detection the RC Sort dashboard uses for FCLM/MonitorPortal: /midway|sign.?in|sentry/i
    function ltLooksLikeAuthWall(r, finalUrl){
        const kioskHost = ltShortUrl(KIOSK_URL);
        const finalHost = ltShortUrl(finalUrl);
        if (finalHost && finalHost !== kioskHost && /midway|sso|auth|login|federate|signin|idp|sentry/i.test(finalHost)) return true;
        const body = (r && r.responseText) || '';
        if (!body) return false;
        if (/laborTracking|calmCode|trackingBadgeId/i.test(body)) return false; // real kiosk page
        return /midway|sign.?in|sentry|authentication required/i.test(body.slice(0, 5000));
    }

    function ltOpenTab(url){
        try { if (typeof GM_openInTab === 'function'){ GM_openInTab(url, {active:true, insert:true, setParent:true}); return; } } catch(e){}
        window.open(url, '_blank');
    }

    // Guided fix — must be triggered from a real click so the tab isn't popup-blocked.
    function ltShowAuthHelp(){
        toast('warning','Opening FCMenu — sign in with Midway, then click your code again.');
        ltOpenTab(KIOSK_URL);
    }

    function ltConfigureFcMenu(){
        const cur = GM_getValue('lt_fcmenu_base', 'https://fcmenu-iad-regionalized.corp.amazon.com');
        const next = prompt(
            'FCMenu base URL (region-specific).\n' +
            'Currently using:\n  ' + cur + '\n\n' +
            'Examples:\n' +
            '  https://fcmenu-iad-regionalized.corp.amazon.com\n' +
            '  https://fcmenu-<region>-regionalized.corp.amazon.com\n\n' +
            'Reload the page after changing.',
            cur
        );
        if (next && next.trim()){
            GM_setValue('lt_fcmenu_base', next.trim().replace(/\/+$/,''));
            toast('success','Saved. Reload the page to apply the new FCMenu URL.');
        }
    }

    // Manual connection test with detailed, actionable reporting.
    function ltTestConnection(){
        toast('info','Testing FCMenu connection…');
        ltUpdateConnDot('yellow');
        console.log('[LaborTracker] Testing FCMenu at', KIOSK_URL);
        GM_xmlhttpRequest({
            method:'GET', url:KIOSK_URL, withCredentials:true, timeout:FCMENU_TIMEOUT,
            headers:{'Accept':'text/html,application/xhtml+xml'},
            onload:r=>{
                const finalUrl = r.finalUrl || KIOSK_URL;
                console.log('[LaborTracker] Test result:', {status:r.status, finalUrl, bytes:(r.responseText||'').length});
                if (ltLooksLikeAuthWall(r, finalUrl)){
                    ltUpdateConnDot('red');
                    toast('warning','Reached FCMenu, but Midway sign-in is required.');
                    ltShowAuthHelp();
                } else if (r.status >= 200 && r.status < 300){
                    ltUpdateConnDot('green');
                    toast('success','✓ FCMenu reachable and authenticated.');
                } else {
                    ltUpdateConnDot('red');
                    toast('error',`FCMenu returned HTTP ${r.status} (${ltShortUrl(finalUrl)}).`);
                }
            },
            onerror:e=>{
                ltUpdateConnDot('red');
                console.error('[LaborTracker] Test network error:', e, 'url=', KIOSK_URL);
                toast('error','Network error — not on corp/VPN, wrong host, or blocked redirect. See console.');
                ltShowAuthHelp();
            },
            ontimeout:()=>{ ltUpdateConnDot('red'); toast('error','Timeout — check VPN / corp network.'); }
        });
    }

    function ltInitSession() {
        ltUpdateConnDot('yellow');
        console.log('[LaborTracker] Init session → GET', KIOSK_URL);
        GM_xmlhttpRequest({ method:'GET', url:KIOSK_URL, withCredentials:true, timeout:FCMENU_TIMEOUT, headers:{'Accept':'text/html,application/xhtml+xml'},
            onload:r=>{
                const finalUrl = r.finalUrl || KIOSK_URL;
                if (ltLooksLikeAuthWall(r, finalUrl)) {
                    ltUpdateConnDot('red');
                    console.warn('[LaborTracker] FCMenu bounced to auth wall:', finalUrl);
                    toast('error','FCMenu needs Midway sign-in — click "Test FCMenu" to fix.');
                    return;
                }
                if (r.status >= 200 && r.status < 300) { ltSubmitCalmCode(r.responseText); }
                else if (r.status === 401 || r.status === 403) { ltUpdateConnDot('red'); toast('error',`FCMenu auth error (${r.status}) — sign in via "Test FCMenu".`); }
                else { ltUpdateConnDot('red'); console.error('[LaborTracker] FCMenu GET status', r.status, finalUrl); toast('error',`FCMenu error (${r.status}) @ ${ltShortUrl(finalUrl)}`); }
            },
            onerror:e=>{ ltUpdateConnDot('red'); console.error('[LaborTracker] FCMenu GET network error:', e, 'url=', KIOSK_URL); toast('error','Cannot reach FCMenu — VPN/host/redirect. Click "Test FCMenu".'); },
            ontimeout:()=>{ ltUpdateConnDot('red'); toast('error','FCMenu timed out — check VPN / corp network.'); }
        });
    }
    ...
    function ltSubmitCalmCode(pageHtml) {
        ...
        GM_xmlhttpRequest({ method:'POST', url:KIOSK_URL, withCredentials:true, timeout:FCMENU_TIMEOUT, headers:{'Content-Type':'application/x-www-form-urlencoded','Accept':'text/html,application/xhtml+xml','Referer':KIOSK_URL}, data:formData,
            onload:r=>{
                const finalUrl = r.finalUrl || KIOSK_URL;
                if (ltLooksLikeAuthWall(r, finalUrl)) { ltSessionReady=false; ltUpdateConnDot('red'); toast('error','FCMenu needs Midway sign-in — click "Test FCMenu".'); return; }
                if(r.status>=200 && r.status<300){ if(r.responseText.includes('trackingBadgeId')||!r.responseText.includes('calmCode')){ ltSessionReady=true; ltUpdateConnDot('green'); } else ltTryAlternateSubmit(r.responseText); }
                else { ltUpdateConnDot('red'); console.error('[LaborTracker] calmCode POST status', r.status, finalUrl); }
            },
            onerror:()=>{ ltUpdateConnDot('red'); toast('error','Failed to submit CALM code'); },
            ontimeout:()=>{ ltUpdateConnDot('red'); toast('error','CALM code submit timed out.'); }
        });
    }
    ...
    function ltTryAlternateSubmit(pageHtml) {
        ...
        GM_xmlhttpRequest({ method:'POST', url:fullUrl, withCredentials:true, timeout:FCMENU_TIMEOUT, headers:{'Content-Type':'application/x-www-form-urlencoded','Referer':KIOSK_URL}, data:formData,
    ...
    function ltTrackBadge(badgeId, name, login, card) {
        ...
        GM_xmlhttpRequest({ method:'POST', url:KIOSK_URL, withCredentials:true, timeout:FCMENU_TIMEOUT,
            ...
            onload:r=>{ ltProcessing.delete(badgeId); card.classList.remove('lt-tracking');
                const finalUrl = r.finalUrl || KIOSK_URL;
                if (ltLooksLikeAuthWall(r, finalUrl)) { ltSessionReady=false; ltUpdateConnDot('red'); card.classList.add('lt-track-failed'); ltAddBadgeIndicator(card,'fail','✗'); toast('error',`✗ ${name} — FCMenu needs Midway sign-in`); ltRecordBadge(badgeId,name,login,false); setTimeout(()=>card.classList.remove('lt-tracked','lt-track-failed'),5000); ltUpdateCount(); return; }
                const success=(r.status>=200&&r.status<300)&&ltParseResponse(r.responseText,badgeId);
            ...
            ontimeout:()=>{ ltProcessing.delete(badgeId); card.classList.remove('lt-tracking'); card.classList.add('lt-track-failed'); ltAddBadgeIndicator(card,'fail','✗'); toast('error',`✗ ${name} — timed out`); ltRecordBadge(badgeId,name,login,false); ltUpdateCount(); }
        });
    }
    ...
    console.log('✅ Zone-RA Unified Suite v2.3 loaded!');
    console.log('🎯 Modules: Labor Tracker | Enhancement Suite | Staffing Lookup');
    console.log('🔌 FCMenu endpoint:', KIOSK_URL);
    console.log('📋 All tools on left sidebar — use "Test FCMenu" in the Labor Track panel to diagnose connectivity');
})();
