// ==UserScript==
// @name         OB Period Report Card V2.2 Hybrid Bridge
// @namespace    http://tampermonkey.net/
// @version      2026-06-16.1
// @description  V2.2 Hybrid bridge. Scrapes NEO/FCLM/MonitorPortal/Fluid Roster as before. On the dashboard it now stays HEADLESS when the page has the native Midway proxy bridge (no floating black box), and yields pulling to the dashboard. Still serves as the fallback bridge when the proxy is down.
// @author       JR
...
(function () {
  ...
  // V2.2: the dashboard HTML now ships its own native Midway proxy bridge and
  // sets this flag. When present, this userscript stays headless on the
  // dashboard (no floating panel) and lets the page own pulling, while still
  // serving as the fallback bridge if the proxy is down.
  const nativeDashboard = () => isDashboard && document.documentElement.dataset.prcNativeBridge === '1';
  ...
  function createPanel(){
    ...
    // V2.2: the dashboard owns a native Midway proxy bridge — stay headless
    // there so there is no floating black box (the dashboard shows status).
...
const row=(label,value)=>
  ...
  function boot(){
    const headlessDash = nativeDashboard();

    createPanel(); // no-ops on the native dashboard
    ...
    if(isDashboard){
      // Keep serving pull requests as the fallback bridge (used when the
      // dashboard's native Midway proxy is offline / not authenticated).
    ...
    // Autonomous dashboard auto-pull loops. Skip when the dashboard owns its
    // own native Midway proxy bridge, so the two don't pull on top of each
    // other (the page drives pulling there).
    if(isDashboard && !headlessDash){
